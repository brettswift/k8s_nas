...
* **BUD-83:** add SEARXNG_BASE_URL env var to OpenClaw deployment, clean stale searxng plugin entries on startup ([b49978c](https://github.com/brettswift/k8s_nas/commit/b49978c8d3034e42e072802ec6ccd1a2e874689e))
* **BUD-83:** debug init container logs, minimal config ([b08389f](https://github.com/brettswift/k8s_nas/commit/b08389f88e69362b23c1f461de3bf18d6e413831))
* **BUD-83:** init container copies settings.yml from ConfigMap to writable volume\n\n- Init container copies /etc/searxng-defaults/settings.yml -> /etc/searxng/settings.yml\n- then chowns to searxng user\n- ConfigMap has proper settings with api_response_format=json ([ca5d089](https://github.com/brettswift/k8s_nas/commit/ca5d089ba59aa9297979cdb58550bccda2f60f3c))
* **BUD-83:** init container writes settings.yml from ConfigMap env vars, removes PVC for config ([2963a5e](https://github.com/brettswift/k8s_nas/commit/2963a5e0e04d065e265ff848df894feb2fdf03de))
* **BUD-83:** minimal settings.yml, init container logs ([ac4df06](https://github.com/brettswift/k8s_nas/commit/ac4df06accd57836a609ef0a65a7a5f6bcf9b872))
...
* override SEARXNG_PORT env var to prevent ArgoCD manifest generation from picking up Kubernetes-injected tcp:// address ([c510070](https://github.com/brettswift/k8s_nas/commit/c5100704f5a19f251a297b3d6f5eedef62e10fd3))
* remove missing namespace.yaml from searxng kustomization resources ([6be8369](https://github.com/brettswift/k8s_nas/commit/6be8369d18c470eb87c43f905689ecba2a4ae802))
...
* **BUD-83:** enable JSON API for SearXNG\n\n- Add SEARXNG_JSON=1 and SEARXNG_FORMATS=html json env vars\n- Mount searxng-settings ConfigMap as /etc/searxng\n- Add configmap.yaml to kustomization ([b234b27](https://github.com/brettswift/k8s_nas/commit/b234b27eb5d726f978db83dc4427160d63005544))
* **BUD-83:** enable JSON API in SearXNG via env vars ([c86d267](https://github.com/brettswift/k8s_nas/commit/c86d26711e9241eb8ca5ca12a63c0d641e5edc49))
* **BUD-83:** force SEARXNG_JSON=1 and USE_DEFAULT_SETTINGS=false in env\n\n- Add env vars for JSON API and custom settings\n- Inline SEARXNG_SECRET directly (no secret ref needed)\n- Pod will restart after ArgoCD sync ([ee059fc](https://github.com/brettswift/k8s_nas/commit/ee059fc70be616f67cd3858bb8ae3036418337cc))
* **BUD-83:** full settings.yml ConfigMap for JSON API\n\n- Replace env-only config with full settings.yml in ConfigMap\n- use_default_settings: false\n- search.api_response_format: json\n- search.formats: [html, json]\n- Pod will restart on ArgoCD sync ([951a317](https://github.com/brettswift/k8s_nas/commit/951a317eb07df25abf9a57bb1496f7d5278d96e8))
...
# [1.12.0](https://github.com/brettswift/k8s_nas/compare/v1.11.3...v1.12.0) (2026-03-30)
...
# [1.12.0](https://github.com/brettswift/k8s_nas/compare/v1.11.3...v1.12.0) (2026-03-29)
