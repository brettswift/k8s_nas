...
- **Whole home on PVC:** The PVC is mounted at `/home/node`, so `~/.openclaw`, `~/.ssh`, `~/.bashrc`, etc. all persist. On first deploy with this layout, the `ensure-workspace-perms` init migrates legacy content (openclaw.json, workspace, .env) into `~/.openclaw/`; `.ssh` stays at `~/.ssh`.
- **Init vs sidecar:** The deployment uses **two** init containers: `bootstrap-openclaw-config` (copy RTK plugin from ConfigMap + merge `openclaw.json` every start — Control UI defaults, searxng cleanup, rtk-rewrite) and `ensure-workspace-perms` (root: migration, crash restore if the sentinel remains, `chown`/`chmod`, SSH key modes). The `config-sentinel` **sidecar** waits for `/healthz` then removes the startup sentinel — it is not a throwaway init; do not remove it without changing the crash-restore flow.
- **Git-tracked config:** The canonical **`k8s_openclaw.json`** lives in the **`buddy_vault`** repo (`buddy_vault/backup/k8s_openclaw.json`). It is **not** stored in this repo and **not** shipped as a full ConfigMap (only the PVC holds the live file; `openclaw-golden-config` here is a minimal disaster fallback, not a sync of that JSON). Copy that file to `~/.openclaw/backup/k8s_openclaw.json` on the PVC and run [`backup/setup-openclaw-json-symlink.sh`](../../../backup/setup-openclaw-json-symlink.sh) (renames the current file to `openclaw.json_bak_initial_simlink_setup` and symlinks `openclaw.json` → `backup/k8s_openclaw.json`).
